...
<script>
    ...
    function initList() {
        $.post("/workflow/getMyForms", function (data) {

            if (data.status == 1) {
                var html = '<thead><tr> <th>桌游</th><th>当前拥有人</th><th>当前步骤</th><th>申请时间</th></tr></thead>';
                html += '<tbody>';
                var res = data.data;
                for (var i = 0; i < res.length; i++) {
                    html += '<tr onclick="toFormDetail(\'' + res[i].uuid + '\')">' +
                        '<td>' + res[i].bgName + '</td>' +
                        '<td>' + res[i].curUserName + '</td>' ;
                    if(res[i].curStep==0){
